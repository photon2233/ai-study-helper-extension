...
- **Multi-Session History** - Create, switch, rename, and delete multiple chat sessions through a foldable drawer; titles are auto-generated by AI after the first reply
- **In-App API Key** - Configure your Gemini API key directly in the extension settings, no environment variables required
...
The backend no longer requires a pre-set API key to start. You can still set `GEMINI_API_KEY` as an environment variable to use as a server-side fallback, but the recommended flow is to configure the key inside the extension UI (see below).
...
2. Click the **⚙️** button in the top-right of the header
3. Paste your Gemini API key (get one from [Google AI Studio](https://aistudio.google.com/app/apikey)) and click **Save**

The key is stored in `chrome.storage.local` and is sent only to the local backend at `127.0.0.1`. It is masked in the settings panel (e.g. `AIza****abcd`) and is never logged. Use the **Clear** button to remove it.
...
1. Click the **🕐** button in the top-right of the header to open the session drawer
2. Click **+ 新会话** to start a fresh conversation
3. Click any session in the list to switch to it
4. Click **✕** on a session to delete it
5. After the first AI reply in a new session, the title is auto-generated to summarize the topic

The **Clear** link at the bottom only clears the messages of the current session — it does not delete the session itself. Sessions persist across popup closes and browser restarts via `chrome.storage.local`.
...
- **Per-Request API Key** - `api_key` is optional in the request body; if absent, the backend falls back to the `GEMINI_API_KEY` env var. A new `genai.Client` is created per request, so different users can share one backend safely.
